...
---
### 📦 Ejemplo completo: `conversion-tipos.ts`

Conversión explícita de tipos y JSON tipado.

```typescript

/**
 * Conversion de Tipos y JSON Tipado
 * -------------------------------------------------------------------
 * - Conversion explicita: String(), Number(), Boolean()
 * - JSON stringify/parse tipados con `as`
 * - Trucos rapidos de conversion controlada
 *
 * (Basico para Tauri: `as`, `JSON.parse` tipado.
 *  Los type guards, `??`, operadores logicos y ternario se ven
 *  en bloques posteriores del curso.)
 */
