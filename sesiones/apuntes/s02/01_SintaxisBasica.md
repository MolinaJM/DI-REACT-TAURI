...
TypeScript es un superconjunto tipado de JavaScript. Toda la sintaxis de JavaScript es válida, pero TypeScript añade **anotaciones de tipo** (`tipo:`) que el compilador `tsc` valida en tiempo de compilación y que luego se eliminan (en la mayoría de los casos) al ejecutar. Esto es lo que se llama "Erasable Syntax". Este capítulo parte del contenido original de JavaScript y lo enriquece con tipos.
...
// dato.edad;            // Error: Object is of type 'unknown'
if (typeof dato === "object" && dato !== null) {
  // aquí `dato` ya está estrechado a object y se puede usar
}
```

**`never`: indica que una función jamás termina con éxito.** Se usa en funciones que solo lanzan errores (el programa aborta antes de devolver nada).
...
function area(forma: Forma): number {
  switch (forma) {
    ...
    default:
      const _nunca: never = forma;  // si falta un caso, ¡error de compilación!
      return _nunca;
  }
}
```
...
Una **aserción de tipo** le dice a TypeScript: "confía en mí, yo sé que este valor es de este tipo". TypeScript **no hace ninguna comprobación en runtime**; solo cambia lo que el compilador cree. Por eso se usan con moderación: si te equivocas, el código compila pero falla en ejecución.
...
**2. `<tipo>valor` (sintaxis angular)** — equivalente a `as`, pero **no funciona en ficheros `.tsx`** ni cuando choca con genéricos de JSX. Por eso `as` es la única que siempre compila en React.
...
**3. `!` (non-null assertion)** — le dice a TS "este valor no es `null` ni `undefined` aunque el tipo lo diga". En runtime **no hace nada**: no elimina el `null`, solo le quita la advertencia al compilador. Úsalo con moderación.
...
**4. `as const` (literal inmutable)** — convierte un objeto o array en un tipo literal de solo lectura. Ideal para configuraciones, rutas o listas que no van a cambiar.

```typescript
const CONFIG = { servicio: "api", version: 3 } as const;
// Tipo: { readonly servicio: "api"; readonly version: 3 }
// CONFIG.version = 4;  // Error: readonly
...
> [!TIP]
> `as` y `as const` son *erasable-only*: no generan código en runtime. `!` tampoco genera código, pero **sí puede causar errores en ejecución** si tu suposición es falsa (el valor sí es `null`). Siempre que puedas, prefiere narrowing (`if (valor !== null)`) sobre `!`.
...
Tipos especiales (any, unknown, void, never) y aserciones de tipo (as, <tipo>, !, as const).
