...
- **Clases mejoradas**: Con modificadores de acceso y más (aunque aquí no vamos a trabajar con clases/objetos normales)
...
3. **Documentación viva**: Los tipos sirven como documentación en el sentido de que explican qué tipos se deben utilizar tanto en variables como en firmas de funciones.
...
# 1. Inicializar el proyecto node (crea el archivo maestra package.json). La -y se salta las preguntas...
npm init -y

# 2. Instalar TypeScript solo en desarrollo (-D) y tsx. Crea node_modules con TS.
...
**Transpilar / ejecutar TypeScript en este curso:**

```bash
# 5. Ejecutar con tsx sin compilar (lo procesa en memroia). Ideal para pruebas rápidas 
npx tsx saludo.ts

# 6. Transpila (alto nivel --> alto nivel) todos los .ts a .js según tsconfig.json (transpila todo el proyecto)
npx tsc
...
console.log("Seguro que yo me jeecuto antes..");
...
const resultado: number = numeros
  .filter((n) => n % 2 === 0) // [2,4]
  .map((n) => n * 2) // [4,8]
  .reduce((acc, n) => acc + n, 0); //acc es un acumulador empieza en 0. 0+4=4, 4+8=12
...
// ESTE TIPO LITERAL SE DESARROLLARÁ MÁS ADELANTE:
// - `type` alias con uniones → 01_SintaxisBasica.md §1.3
type Operador = "+" | "-" | "*";

//Función que a su vez devuelve otra función
...
// 2. Usamos la función recién creada
console.log(sumar(5, 3)); // Resul: 8
console.log(sumar(10, 20)); // Resul: 30

// Creamos otra función específica para multiplicar
...
// 1. Promesas con Fetch directo a SWAPI (sin try-catch)
async function cargarPersonajes(): Promise<PersonajeSW[]> {
  ...
  const personajes = validarPersonajes(bruto);//VALIDACIÓN
...
async function obtenerPersonajesAltos(): Promise<PersonajeSW[]> {
  try {
    ...
    const bruto: unknown = await response.json();
    const personajes: PersonajeSW[] = validarPersonajes(bruto);//VALIDACIÓN
...
graph TD
    ...
    subgraph Tecnicas_Consumo ["Técnicas de Consumo"]
        Decision -->|Técnica 1 y 2: Individual| Single["cargarPersonajes() / obtenerPersonajesAltos()<br>Procesa 1 solo endpoint"]:::tech
...
function saludarTradicional(nombre: string): string {
  return `Hola, ${nombre}`; // ESTE CONCEPTO SE DESARROLLARÁ MÁS ADELANTE: template literal → 00_Introduccion.md §0.5
}
...
// - Props y Hooks de React → sesiones S04+
...
// ESTE CONCEPTO SE DESARROLLARÁ MÁS ADELANTE:
// - `interface` → 01_SintaxisBasica.md §1.3
// - `?` (propiedad opcional) → 01_SintaxisBasica.md §1.3
...
//  ESTE CÓDIGO PRESENTA CONCEPTOS QUE SE DESARROLLARÁN MÁS ADELANTE:
// - `interface` → 01_SintaxisBasica.md §1.3
// - `|` (unión de tipos) → 01_SintaxisBasica.md §1.3
// - `"propiedad" in objeto` → 05_ControlDeFlujo.md §5.4 (narrowing)
interface Alumno {
  nombre: string;
  edad: number;
...
// ESTE CÓDIGO PRESENTA CONCEPTOS QUE SE DESARROLLARÁN MÁS ADELANTE:
// - `any` → 01_SintaxisBasica.md §1.2 (tipos especiales)
...
```bash
1-Guardar el anterior bloque de código como error.ts
...
3-Obtenemos el mismo resultado que si lanzamos con: npx tsx error.ts
4-Pero, si COMPILAMOS diciendo que no genere el .js, entonces SÍ marca los errores. Probamos:
  npx tsc error.ts --noEmitOnError --ignoreConfig  
  -Con --ignoreConfig compila un fichero (y no todo el proyecto). 
  -Además, con --noEmitOnError: No compila si encuentra error 
