...
| 6 · Type Assertions | `s02/09` | **P3** | `as`/`as const` con `invoke`, refs y constantes. |
...
3. Imagina que recibes datos de una API y necesitas convertirlos. Convierte un `number` a `string`, `boolean` y `number` de nuevo con `String()`, `Boolean()` y `Number()`.
...
5. Estás configurando un producto en una tienda online. Declara `precio: number = 99.99` con tipado explícito y `activo` con `true` dejando que TypeScript infiera el tipo. Compara: ¿qué diferencia hay entre `precio: number = 99.99` y `activo = true`?
6. Estás construyendo un formulario de registro. Crea `nombre` y `apellido` (ambos `string`) y combínalos con una template literal en `nombreCompleto`.
7. Estás trabajando con un sistema que procesa números y necesita saber cuántos dígitos tiene cada uno. Escribe `longitudTexto(numero: number): number` que devuelva cuántos dígitos tiene el número (pista: conviértelo a `string` primero y usa `.length`).
...
4. Estás implementando un manejador de estados que debe cubrir todos los casos posibles. Implementa un switch exhaustivo con un tipo unión y una variable `never` en el `default`.
5. Estás construyendo un sistema de logging. Escribe `logMensaje(texto: string): void` que imprima el mensaje con `console.log`.
6. Estás procesando datos que pueden ser de cualquier tipo. Crea `procesarDesconocido(valor: unknown)`: si es `string`, imprime su longitud; si no, imprime `"No es un texto"`.
7. Estás implementando un manejador de errores que nunca devuelve control. Escribe `lanzarError(mensaje: string): never` que lance `new Error(mensaje)`.
8. Estás refactorizando código legacy que usa `any` por comodidad. Crea una función `duplicar` que reciba un valor y lo duplique, pero sin usar `any`: usa un type guard o `typeof` para verificar que el valor es un número antes de multiplicarlo por 2.
9. Estás diseñando un sistema de formas geométricas y quieres garantizar que ningún caso quede sin cubrir. Dado `type Forma = "circulo" | "cuadrado"`, escribe una función con `switch` cuyo `default` asigne el parámetro a una variable `never` para garantizar el exhaustive checking.
...
1. Estás recibiendo datos JSON de una API y necesitas convertirlos a un tipo tipado. Usa `as` para estrechar un valor `unknown` procedente de `JSON.parse` (una API / una llamada que devuelve JSON) y conviértelo a la interface `Pelicula`. ¿Qué ganancia de tipo aporta `as`? ¿Qué riesgo tiene?
...
2. Estás persistiendo datos de un objeto en localStorage o enviándolos a un servidor. Usa `JSON.stringify` y `JSON.parse` con una interface tipada.
3. Estás construyendo un formulario donde algunos campos pueden venir vacíos y necesitas valores por defecto. Escribe una función que use el operador `??` para proporcionar valores por defecto.
4. Estás trabajando con valores que pueden ser `0`, `false` o `""` y necesitas que el operador no los considere falsy. Diferencia entre `||` y `??` con un ejemplo donde `0` sea un valor válido.
...
2. Estás filtrando una lista de elementos que pueden ser de diferentes tipos y solo quieres los de un tipo concreto. Usa un type predicate en un `filter` para obtener solo los elementos de un tipo concreto de una unión.
...
4. Estás optimizando el rendimiento de una función costosa guardando sus resultados. Crea `memoizar(fn)` que guarde resultados en un `Map` para argumentos repetidos.
...
6. Estás accediendo a propiedades de un objeto de forma dinámica y necesitas que el tipo de la clave sea verificado. Escribe un acceso indexado seguro: `obtenerValor<T, K extends keyof T>(obj, key)`.
...
2. Estás definiendo una clase y olvidaste inicializar una propiedad. Declara una propiedad de clase sin inicializar — ¿qué error da `strictPropertyInitialization`?
...
4. Estás calculando el puntaje total de una lista de números sin permitir que la lista original se modifique. Escribe una función que reciba un `ReadonlyArray<number>` y devuelva su suma.
...
2. Estás extrayendo datos específicos de un objeto anidado y necesitas hacerlo de forma concisa. Usa destructuring avanzado: alias, anidado y valores por defecto.
...
4. Estás agrupando productos por categoría para mostrarlos en secciones. Usa `Object.groupBy` para agrupar productos por categoría.
