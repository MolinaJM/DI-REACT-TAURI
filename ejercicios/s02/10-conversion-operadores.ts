...
// Catálogo: sesiones/ejerciciosTS.md · Sesión 2 · Bloques 14 y 15
...
// 4) `??=` y `||=`: asignan solo si no hay valor previo
export const cache: Record<string, string> = {};
export function asegurarCache(clave: string): string {
  // TODO: `cache[clave] ??= "creado"`
  return cache[clave] || "creado";
}
export function marcarPreparado(preparado: boolean): boolean {
  // TODO: `preparado ||= true`
  return preparado;
}

// 5) Short-circuit `&&`: no ejecuta la llamada si la condición falla
let veces = 0;
...
// 6) `JSON.stringify` / `JSON.parse` tipados
